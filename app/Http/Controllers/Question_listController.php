...
{
    public function index(Request $request, $situation='recruting', $page='1')
    {
        if($situation === 'recruting')
        {
            $items = Question::recruting()->orderBy('id', 'desc')->offset(($page-1)*30)->limit(30)->get();
            $makePageLinks = Functions::makePageLinks(Question::recruting()->count(), $page);


        }elseif($situation === 'voting')
        {
            $items = Question::voting()->orderBy('id', 'desc')->offset(($page-1)*30)->limit(30)->get();
            $makePageLinks = Functions::makePageLinks(Question::voting()->count(), $page);

        }elseif($situation === 'finished')
        {
            $items = Question::finished()->orderBy('id', 'desc')->offset(($page-1)*30)->limit(30)->get();
